...
#[inline]
fn drop_batch(rx: &mut RxRing, drop: &mut DropRing, descs: &mut [RattanDesc]) -> usize {
    let received = rx.consume(descs);

    let tokens: &mut [u64] = &mut descs[..received]
        .iter()
        .map(|d| d.token)
        .collect::<Vec<_>>();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (mut fill0, mut drop0, mut rx0, mut tx0) = dev0.register_rings_for_queue(0, rings0)?;
    ...
    if CPU_NAPI_DEV0 >= 0 {
        println!(
            "   rattan0 NAPI CPU: {} (client-side injection)",
            CPU_NAPI_DEV0
        );
    } else {
        println!("   rattan0 NAPI: use caller's CPU");
    }
    if CPU_NAPI_DEV1 >= 0 {
        println!(
            "   rattan1 NAPI CPU: {} (server-side injection)",
            CPU_NAPI_DEV1
        );
    } else {
    ...
    let packets_dropped_clone = packets_dropped.clone();
            ...
            for i in 0..nfds as usize {
                ...
                if dev_idx == 0 {
                    ...
                    loop {
                        ...
                        if round_count & 15 == 0 && round_count > 100 {
                            // Receive and drop!
                            let drop = drop_batch(&mut rx0, &mut drop0, &mut descs);
                            packets_dropped_clone.fetch_add(drop as u64, Ordering::Relaxed);
                            continue;
                        }
    ...
    let total_dropped = packets_dropped.load(Ordering::Relaxed);
    println!("   Packets dropped: {}", total_dropped);

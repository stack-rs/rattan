...
where
    P: Packet,
{
    pub fn new(config: TokenBucketCellConfig) -> Result<TokenBucketCell<P>, Error> {
...
mod tests {
    ...
    fn get_delays(
        ...
        let cell = TokenBucketCell::new(config)?;
        ...
        for _ in 0..number {
            ...
            let test_packet = TestPacket::<StdPacket>::from_raw_buffer(buf, logical_time);
    ...
    fn test_token_bucket_cell_queue() -> Result<(), Error> {
        ...
        let cell = TokenBucketCell::new(TokenBucketCellConfig::new(
            1024,
            Bandwidth::from_bps(4096),
            ByteSize::b(512),
            None,
    ...
    fn test_token_bucket_cell_burst() -> Result<(), Error> {
        ...
        for _ in 0..5 {
            let test_packet = TestPacket::<StdPacket>::from_raw_buffer(&[0; 128], burst_time);
            ingress.enqueue(test_packet)?;
        }
        for _ in 0..5 {
            let mut received: Option<TestPacket<StdPacket>> = None;
